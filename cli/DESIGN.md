...
- 対象版: テンプレート 2.4.4（PlantUML 対応。§13）
...
フィルタが `diagrams/mmd-<key>.svg` を決め、中身が SVG ならそのまま使い、exe を呼ばない。
**一括変換（2.4.4 から）**。図を 1 枚ずつ `ddq mermaid` にするとブラウザの起動が図の数だけ走る（1 図 0.9 秒。
100 図の一括なら 1 図 0.27 秒）。そこで 2 段で束ねる:

1. `ddq html` / `ddq pdf` は Quarto を起動する前に、文書（`_quarto.yml` の chapters・appendices と include）の
   mermaid フェンスを集め、フィルタと**同じキー**で `diagrams/mmd-<key>.svg` を 1 回の呼び出しで描く
   （`mermaid::prerender`。フェンスの中身は Pandoc の CodeBlock と同じく、フェンスの字下げを外して LF でつなぐ）。
   Quarto は book を章ファイルごとに描くので、フィルタだけでは章をまたいで束ねられない
2. フィルタは 2 段のフィルタとして返し、1 段目（`Pandoc`）でその章ファイルの mermaid をまとめて描く
   （素の `quarto render` のとき用。1 で描いたものはキャッシュにあるので何もしない）

キーがずれても壊れない（ずれた図はフィルタが 1 枚ずつ描くので遅くなるだけ）。描けなかった図もフィルタが
1 枚ずつ描き直し、図ごとのエラーを出す。サンプル文書（mermaid 22 図）の HTML はキャッシュ無しで
36.7 秒 → 22.4 秒（キャッシュ有りは 14.7 秒）。キーの計算はフィルタ（`cache_key`）と `mermaid::prerender::cache_key` の
2 か所にあり、同じでなければならない（E2E の図の数の照合が、ずれを検出する）。キーは SHA-1 の先頭 16 桁で、図のソースに加えて
**テンプレートの版・`mermaid-config.json` の中身・`DDQ_MERMAID_ENGINE`（未設定なら `auto`）**を混ぜる
