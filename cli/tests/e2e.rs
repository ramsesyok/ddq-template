...
//!
//! examples/docs（mermaid 22 図）を一時フォルダへ写し、`ddq update` → `ddq pdf` → `ddq html`
...
/// quarto が無いときの扱い: DDQ_E2E=1 なら失敗、そうでなければ skip（true を返す）。
fn skip_without_quarto() -> bool {
    ...
    assert!(
        ...
        "DDQ_E2E=1 なのに quarto が PATH にありません"
    );
...
fn build_and_check(dir: &Path) {
    ...
    // PDF で焼いた SVG が diagrams/ にキャッシュされ、mermaid の数と一致する
