...
- 表の表示・セル編集。操作の流儀は Excel / Handsontable（vscode-edit-csv）にそろえる
  - 編集開始: 文字キー（内容を置き換えて開始）、Enter / F2 / ダブルクリック（キャレットは末尾）
  - 編集中: Alt+Enter で改行、Enter で確定して下へ、Tab で確定して右へ、Escape で
    **取り消し**（編集前の値に戻す）。別のセルのクリックや入力欄への移動でも確定する
  - 本文は確定時にだけモデルへ書く（入力途中は draft）。Undo の単位が 1 回の編集になる
  - 入力欄の高さは中身の行数に合わせて自動で伸縮する（改行しても行が隠れない）
  - textarea は Alt+Enter では改行しないので、改行の挿入とキャレット復帰は自前で行う
  - 編集用の `textarea` は常に 1 つだけ置き、選択中は 1px の透明な要素としてアクティブ
    セルの位置でフォーカスを持つ。文字や IME の入力が届いた時点で編集モードへ切り替え、
    同じ要素をセルの上に広げる。要素を差し替えないので IME の変換途中でも入力が途切れない
- 選択と移動
  - アクティブセル（キー操作の起点）と矩形範囲。結合セルは 1 つの単位として扱い、
    移動では飛び越え、範囲は結合セル全体を含むまで広げる
  - 移動: 矢印キー、Tab / Shift+Tab（行末で折り返す）、Home / End、Ctrl+Home / Ctrl+End
  - 範囲選択: ドラッグ、Shift+クリック、Shift+矢印（広げる向きの結合セルは端まで飛ぶ）、
    選択の右下の ■（フィルハンドル）のドラッグ、Ctrl+A。Escape で解除
  - フィルハンドルは**選択を広げるだけ**で値はコピーしない（Excel のオートフィルとは違う）
  - セル本文はマウスで選択できない（`user-select: none`）。ドラッグで範囲選択をするため。
    文字を選びたいときは編集モードに入る（`textarea` は選択可）
- Delete / Backspace で選択範囲の内容を消去（結合は崩さない）
- Undo / Redo（Ctrl+Z ／ Ctrl+Y・Ctrl+Shift+Z、ツールバーのボタン）
  - モデルのスナップショットを積む（上限 100）。セル編集・行列操作・結合・貼り付け・
    属性・列幅・揃えをすべて対象にする。選択は履歴に入れない
  - caption などの入力欄と編集中のセルでは横取りせず、その欄の Undo に任せる
- 行・列の追加 / 削除
  - 追加は**上／下・左／右**を選べる（右クリックメニューとツールバー）。上／左に挿入した
    ときは選択がずれないよう追従させる
  - 削除は選択の先頭行／先頭列の 1 つだけ（複数選択でもまとめて消さない）
- 右クリックメニュー: 行の挿入（上／下）・削除、列の挿入（左／右）・削除、結合・解除、
  内容の消去、元に戻す／やり直す。選択の外を右クリックしたときはそのセルを選び直す
- セル結合 / 結合解除
...
- 出力形式セレクタ: グリッド表 / merge-cols。再現不可なら merge-cols を無効化し
  理由をツールチップ表示
- クリップボード（`text/plain` の TSV のみ。`text/html` は扱わない）

  | 操作 | 動作 |
  |------|------|
  | Ctrl+C（選択あり） | 選択範囲を TSV でコピー。結合に吸収されたセルは空 |
  | Ctrl+V（選択あり） | 選択の左上を起点に上書き。値が 1 つなら選択範囲を埋める |
  | Ctrl+V（選択なし） | 表全体を置き換え（Excel からの取り込み） |

  - 改行・タブ・`"` を含むセルは Excel と同じく `"…"` で囲み、内側の `"` は `""`
  - 貼り付けが表からはみ出す分は行・列を足す
  - 結合に吸収されたセルには書き込まない（結合を崩さない）。捨てた数はメッセージで知らせる
  - 選択なしの Ctrl+V は**タブを含むテキストだけ**受け付ける。よそからコピーした文章で
    表を丸ごと潰さないため（選択があるときはこの制限を掛けない＝1 列だけの取り込みもできる）
  - 編集中（`textarea`）の Ctrl+C / Ctrl+V は横取りしない
  - Excel の「クリップボードの形を選択範囲へ繰り返し敷き詰める」挙動は再現しない
    （値 1 つの塗りつぶしまで）
- 生成 Markdown のプレビュー
- Apply でドキュメントへ反映
...
src/
  ...
  model/
    ...
    selection.ts             # アクティブセルの移動・範囲の拡張（結合セルを 1 単位として扱う）
    clearCells.ts            # 範囲の内容消去
...
- 対象ファイル: `.qmd` / `.md`
- コマンド: `ddqTableEditor.open`（DDQ Table Editor: Open Table Editor）
