...
**確認済み（静的・限定実行）**：PDF は `--to typst --profile publish`、HTML は `--to html` を渡す。出力先は `_quarto.yml` の `output-dir`（PDF は `_quarto-publish.yml` の指定を優先。既定 `_book`）を `quarto::output_dir` が読む。HTMLでは `MERMAID_SVG=1` により図をSVG化する。PDFは出力先の直下の拡張子PDFを大小文字を区別せず収集し、ちょうど一件の場合だけ `design-doc.pdf` にコピーする。ゼロ件・複数件ならエラーになる。2.4.2までは `_book` 決め打ちで、output-dir を変えると取り出せなかった（U-0006で修正）。[E-0007](appendix-evidence.qmd#ev-e-0007)
