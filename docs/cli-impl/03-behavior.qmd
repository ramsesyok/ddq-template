...
**確認済み（静的・限定実行）**：PDF は `--to typst --profile publish`、HTML は `--to html` を渡す。出力先は `_quarto.yml` の `output-dir`（PDF は `_quarto-publish.yml` の指定を優先。既定 `_book`）を `quarto::output_dir` が読む。HTMLでは `MERMAID_SVG=1` により図をSVG化する。どちらも Quarto の前に `mermaid::prerender` が文書全体の mermaid を1回の変換で描き、フィルタと同じ名前でキャッシュに置く（2.4.4）。PDFは出力先の直下の拡張子PDFを大小文字を区別せず収集し、ちょうど一件の場合だけ `design-doc.pdf` にコピーする。ゼロ件・複数件ならエラーになる。2.4.2までは `_book` 決め打ちで、output-dir を変えると取り出せなかった（U-0006で修正）。[E-0007](appendix-evidence.qmd#ev-e-0007)
...
**確認済み（静的）**：出力は `<out-dir>/quarto-template-<埋込み版>/` と同名ZIP。既存の同名配布フォルダは削除して再作成し、実行中のddq自身、README、AGENT-GUIDE、`LICENSE`、`THIRD-PARTY-NOTICES.md`、jar、はじめかたPDF、マニュアルPDF／HTML、拡張ごとのVSIXを集める。一式は隣の作業フォルダ（`<名前>.building`）に集め、全部そろってから前回の配布フォルダと入れ替える（途中で失敗しても前回は壊れない）。重いビルドの前に `THIRD-PARTY-NOTICES.md` の存在と、先頭に記録した入力（`Cargo.lock`・`mermaid.min.js`・`plantuml.jar`・拡張の `package-lock.json`）のSHA-1（テキストは改行をLFに揃えて計算）が今のファイルと一致することを確かめ、違えば止める。`--with-sample` では `examples/docs` を `docs/` としてコピーし、生成物・PDF側補助・図キャッシュを除く。除外条件はコピー中のファイル名／接頭辞に適用される。[E-0013](appendix-evidence.qmd#ev-e-0013)
